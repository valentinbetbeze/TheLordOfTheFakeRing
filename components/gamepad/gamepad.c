...
void gamepad_init_joystick(adc_oneshot_unit_handle_t *handle, joystick_t *joystick)
{
    ...
    ESP_ERROR_CHECK(adc_oneshot_io_to_channel(PIN_JOY_Y, &(unit_cfg.unit_id), &(joystick->axis_y.channel)));
    // Configure ADC channels
    ESP_ERROR_CHECK(adc_oneshot_config_channel(*handle, joystick->axis_x.channel, &chan_cfg));
    ESP_ERROR_CHECK(adc_oneshot_config_channel(*handle, joystick->axis_y.channel, &chan_cfg));
}
