...
void update_player_position(game_t *game, player_t *player, const int8_t x_axis_value)
{
    ...
    if ((game->timer - player->timer_x) / TIMESTEP_X > 1) {
        ...
        else if (x_axis_value < 0){
            player->forward = 0;
        }
        player->physics.pos_x += player->physics.speed_x * (int16_t)(x_axis_value);
 ...
 */
static uint8_t check_enemy_player_collision(game_t *game, player_t *player,
                                            enemy_t *enemy)
{
...
void spawn_enemies(const map_t *map, const uint16_t cam_pos_x,
                   const int16_t start_row, const int16_t end_row)
{
    if (map == NULL) {
