 ...
 * behaviours must be modeled. Hence the left @p `l_` and right @p `r_` 
...
/**
 * @brief Filters out the first push of a button from the signal
 * 
 * @param btn Pointer to the button object to process.
 */
void filter_push_signal(button_t *btn);

/**
 * @brief Check if a button has been pressed directly from the GPIO state
 * 
